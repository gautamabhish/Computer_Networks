...
string userName;
unordered_map<string, sockaddr_in> userDirectory;
mutex coutMutex;
mutex userDirMutex;
...
void sendJoin(SOCKET sockfd, const sockaddr_in &broadcastAddr) {
    Packet p{};
    memset(&p, 0, sizeof(p));
    strncpy(p.name, userName.c_str(), sizeof(p.name)-1);
    strncpy(p.message, "/JOIN", sizeof(p.message)-1);
    sendto(sockfd, (const char*)&p, sizeof(p), 0, (const sockaddr*)&broadcastAddr, sizeof(broadcastAddr));
}

// Send NAME_TAKEN warning
void sendNameTaken(SOCKET sockfd, const sockaddr_in &dest) {
    Packet p{};
    memset(&p, 0, sizeof(p));
    strncpy(p.name, "SERVER", sizeof(p.name)-1);
    strncpy(p.message, "/NAME_TAKEN", sizeof(p.message)-1);
    sendto(sockfd, (const char*)&p, sizeof(p), 0, (const sockaddr*)&dest, sizeof(dest));
}
...
// Receiving messages
void receiveMessages(SOCKET sockfd) {
    Packet packet{};
...
// Sending messages
void sendMessages(SOCKET sockfd, sockaddr_in broadcastAddr) {
    string msg;
    Packet packet{};
    memset(&packet, 0, sizeof(packet));
    ...
    while (true) {
        ...
        if (msg[0] == '@') {
            ...
            if (spacePos != string::npos) {
                ...
                if (it != userDirectory.end()) {
                    ...
                    sendto(sockfd, (const char*)&packet, sizeof(packet), 0, (sockaddr*)&it->second, sizeof(it->second));
        ...
        sendto(sockfd, (const char*)&packet, sizeof(packet), 0, (const sockaddr*)&broadcastAddr, sizeof(broadcastAddr));
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (sockfd == INVALID_SOCKET) { cerr << "Socket creation failed\n"; WSACleanup(); return 1; }
    ...
    if (bind(sockfd, (sockaddr*)&localAddr, sizeof(localAddr)) < 0) { cerr << "Bind failed\n"; closesocket(sockfd); WSACleanup(); return 1; }
